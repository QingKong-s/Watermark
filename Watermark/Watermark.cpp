    ...
    _In_ PWSTR pszCmdLine, _In_ int nCmdShow)
{
    ...
    UINT uErr;
    if (const auto iRetInit = eck::Initialize(hInstance, nullptr, &uErr);
        iRetInit != eck::InitStatus::Ok)
    {
        EckDbgPrintFormatMessage(uErr);
        eck::MsgBox(eck::Format(L"Init failed!\r\nInitStatus = %d\r\nError code = %08X",
            (int)iRetInit, uErr), L"Error", MB_ICONERROR);
        return 0;
    }
    ...
    const auto pt = eck::CalculateCenterWindowPosition(nullptr, size.cx, size.cy);
